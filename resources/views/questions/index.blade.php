...
<div class="container mx-auto px-4 py-6">
    <!-- Header and Create Thread Button -->
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Forum Diskusi</h2>
        <a href="{{ route('questions.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md transition duration-200">
            Buat Thread Baru
        </a>
    </div>
    ...
    <form action="{{ route('questions.search') }}" method="GET" class="mb-6 flex">
        <input type="text" name="q" class="flex-grow border border-gray-300 rounded-l-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder="Cari thread..." value="{{ request('q') ?? ($keyword ?? '') }}">
        <button class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-r-md transition duration-200" type="submit">
            Cari
        </button>
    </form>
                ...
                <div class="bg-white rounded-lg shadow-md mb-4 overflow-hidden hover:shadow-lg transition duration-200">
                    <div class="p-6">
                        ...
                        <div class="text-sm text-gray-500 mb-3">
                            Ditanya oleh:
                            <a href="{{ route('users.show', $question->user->id) }}" class="text-blue-600 hover:text-blue-800">
                                {{ $question->user->name }}
                            </a>
                    ...
                    <ol class="list-decimal list-inside space-y-2">
                        @foreach($top_users as $user)
                            <li class="hover:bg-gray-50 p-1 rounded">
                                ...
                                <span class="text-sm text-gray-500">({{ $user->questions_count }} thread)</span>
                            </li>
                        @endforeach
                    </ol>
                </div>
                ...
                <div class="p-4">
                    <ol class="list-decimal list-inside space-y-2">
                        @foreach($top_liked_users as $user)
                            <li class="hover:bg-gray-50 p-1 rounded">
                                <span class="font-medium">{{ $user->name }}</span> 
                                <span class="text-sm text-gray-500">({{ $user->questions->sum('likes_count') }} like)</span>
                            </li>
                        @endforeach
                    </ol>
                </div>
