...
<div class="container">
    ...
    @if ($question->images)
        <div class="mb-3 flex flex-wrap gap-3">
            @foreach ($question->images as $img)
                <img src="{{ asset('storage/question_images/'.$img) }}" style="max-width:220px;max-height:160px;" class="rounded shadow border">
            @endforeach
        </div>
    @endif
    ...
    @foreach ($question->comments as $comment)
        <div class="mb-3">
            ...
            <p>{{ $comment->content }}</p>
            @if ($comment->image)
                <img src="{{ asset('storage/comment_images/'.$comment->image) }}" style="max-width:180px;max-height:120px;" class="rounded shadow border">
            @endif
