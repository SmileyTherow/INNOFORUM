        ...
        <div class="container mx-auto px-4 max-w-4xl">
            ...
            <div class="bg-gray-800 rounded-2xl shadow-xl overflow-hidden border border-gray-700">
                @forelse($users as $user)
                    <div
                        class="flex items-center gap-4 border-b border-gray-700 px-6 py-4 hover:bg-gray-750 transition-colors">
                        <!-- Avatar -->
                        @if ($user->photo)
                            <img src="{{ asset('storage/photo/' . $user->photo) }}"
                                class="w-12 h-12 rounded-full bg-blue-500/20 border border-gray-600 shadow" alt="avatar">
                        @else
                            <div
                                class="w-12 h-12 rounded-full bg-blue-500/20 flex items-center justify-center border border-gray-600">
                        ...
                        <!-- User Info -->
                        <div class="flex-1">
                            @include('components.user-mini-profile', [
                                'user' => $user,
                                'class' =>
                                    'font-semibold text-blue-400 hover:text-blue-300 text-lg ' . $user->name,
                            ])
                            <div class="text-xs text-gray-400 mb-1">
                                Anggota sejak {{ $user->created_at->translatedFormat('M Y') }}
                            </div>
                            <div class="flex gap-2 text-xs">
                                <span class="bg-blue-500/20 text-blue-300 px-2 py-1 rounded border border-blue-500/30">
                                    Komentar: {{ $periode == 'all' ? $user->comments->count() : $user->comments_in_period }}
                                </span>
                                <span class="bg-green-500/20 text-green-300 px-2 py-1 rounded border border-green-500/30">
                                    Pertanyaan:
                                    {{ $periode == 'all' ? $user->questions->count() : $user->questions_in_period }}
                                </span>
                                <span
                                    class="bg-yellow-500/20 text-yellow-300 px-2 py-1 rounded border border-yellow-500/30">
                                    Like:
                                    {{ $periode == 'all' ? $user->comments->sum(fn($c) => $c->likes->count()) : $user->like_count_in_period }}
                                </span>
                            </div>
                        </div>
                        <!-- Points -->
                        <span
                            class="bg-yellow-500/20 text-yellow-300 px-5 py-2 rounded-full font-bold text-base border border-yellow-500/30">
                            {{ $periode == 'all' ? $user->points : $user->period_points }}
                            pts
                        </span>
                    </div>
                @empty
                    <div class="text-center py-6 text-gray-400">Belum ada pengguna dengan poin.</div>
                @endforelse
