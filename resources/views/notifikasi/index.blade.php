...
<div class="max-w-4xl mx-auto py-8 px-4">
    ...
    <div class="flex gap-3 mb-6 flex-wrap">
        <a href="{{ route('notifications.index', ['tab' => 'unread']) }}"
            ...
            Belum Dibaca
            @if($count_unread > 0)
                <span class="ml-2 inline-block bg-red-500 text-white text-xs px-2 py-1 rounded-full">{{ $count_unread }}</span>
            @endif
        </a>
        ...
        <form action="{{ route('notifications.markAllAsRead') }}" method="POST" class="inline">
            @csrf
            <button type="submit" class="px-4 py-2 rounded-lg bg-green-600 hover:bg-green-500 text-white text-sm transition-colors">
    ...
    <div class="space-y-4">
        @forelse($notifications as $notif)
            <div class="p-4 rounded-lg shadow bg-gray-800 border border-gray-700 {{ !$notif->is_read ? 'border-l-4 border-blue-500' : '' }}">
                <div class="flex justify-between items-start">
                    ...
                    <div class="flex gap-2 ml-4">
                        ...
                        <form action="{{ route('notifications.read', $notif->id) }}" method="POST">
                            @csrf
                            <button class="text-green-400 bg-green-500/20 hover:bg-green-500/30 px-3 py-1 rounded transition-colors border border-green-500/30 text-sm">
                                ✔
                            </button>
                        </form>
                        @endif
                        <form action="{{ route('notifications.destroy', $notif->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button class="text-red-400 bg-red-500/20 hover:bg-red-500/30 px-3 py-1 rounded transition-colors border border-red-500/30 text-sm">
                                🗑️
                            </button>
                        </form>
                    </div>
        ...
        @empty
            <div class="p-8 text-center text-gray-400 bg-gray-800 rounded-lg border border-gray-700">
                ...
                <p class="text-lg">Tidak ada notifikasi {{ $tab === 'read' ? 'yang sudah dibaca' : 'yang belum dibaca' }}.</p>
            </div>
        @endforelse
    </div>
